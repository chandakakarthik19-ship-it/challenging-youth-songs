    ...
    <main className="min-h-screen pb-32">
      ...
      <section className="mx-auto grid max-w-7xl gap-10 px-6 pb-16 pt-10 lg:grid-cols-[1.1fr_.9fr] lg:px-10 lg:pt-16">
        <div className="flex flex-col justify-center"><p className="mb-5 flex items-center gap-2 text-xs font-bold uppercase tracking-[0.22em] text-[#e66f2e]"><span className="h-2 w-2 rounded-full bg-[#e66f2e]" /> Festival season 2026</p><h1 className="display-font max-w-2xl text-5xl font-bold leading-[1.02] tracking-[-0.045em] text-[#17221c] sm:text-7xl">Turn it up for <span className="text-[#e66f2e]">Vinayaka.</span></h1><p className="mt-7 max-w-lg text-lg leading-8 text-[#68736c]">Your home for the loudest dhols, warmest chants, and late-night DJ edits of Chavithi.</p><div className="mt-9 flex flex-wrap items-center gap-4"><a href="#library" className="rounded-full bg-[#17221c] px-6 py-3.5 text-sm font-bold text-white transition hover:bg-[#2f6849]">Explore the mixes</a><span className="text-sm font-semibold text-[#68736c]">{songs.length} tracks in the archive</span></div></div>
          <div className="grain relative min-h-[560px] overflow-hidden rounded-[2rem] bg-[#2f6849] p-6 text-white shadow-2xl shadow-[#2f6849]/20 sm:p-8 lg:p-9">
            <div className="absolute -right-16 -top-20 h-72 w-72 rounded-full border-[35px] border-[#f5bd6d]/70" />
            <div className="absolute -bottom-28 -left-16 h-64 w-64 rounded-full border-[28px] border-[#e66f2e]/90" />
            <div className="relative z-10 flex h-full flex-col justify-between">
              <div className="flex items-start justify-between">
                <span className="rounded-full bg-white/15 px-3 py-1 text-[10px] font-bold uppercase tracking-[0.18em]">Nadam sessions / 01</span>
                <Volume2 size={22} />
              </div>
              <div className="pt-8">
                <p className="mb-3 text-sm font-semibold uppercase tracking-[0.18em] text-[#f5bd6d]">Now playing</p>
                <h2 className="display-font max-w-[16ch] text-4xl font-bold leading-[0.98] tracking-[-0.05em] text-white sm:text-[3.4rem]">{activeSong?.title ?? "Choose a mix"}</h2>
                <p className="mt-4 text-base text-white/75">{activeSong?.artist ?? "Your collection"}</p>
                <div className="mt-8">
                  <input aria-label="Seek through current song" type="range" min="0" max={duration || 0} step="0.1" value={Math.min(currentTime, duration || 0)} onChange={(event) => seekSong(Number(event.target.value))} disabled={!duration} className="h-1.5 w-full cursor-pointer accent-[#f5bd6d] disabled:cursor-not-allowed disabled:opacity-50" />
                  <div className="mt-2 flex justify-between text-xs font-semibold text-white/65">
                    <span>{formatTime(currentTime)}</span>
                    <span>{formatTime(duration)}</span>
                  </div>
                </div>
                <div className="mt-6 flex items-center justify-center gap-4 pt-2">
                  <button aria-label="Play previous song" title="Previous song" onClick={playPreviousSong} disabled={!activeSong || !filteredSongs.length} className="grid h-12 w-12 place-items-center rounded-full bg-white/10 text-white transition hover:scale-105 hover:bg-white/15 disabled:cursor-not-allowed disabled:opacity-40"><SkipBack size={18} /></button>
                  <button aria-label={isPlaying ? "Pause current song" : "Play current song"} onClick={togglePlayback} className="grid h-16 w-16 place-items-center rounded-full bg-[#f5bd6d] text-[#17221c] shadow-lg shadow-[#f5bd6d]/40 transition hover:scale-105"><span>{isPlaying ? <Pause size={26} fill="currentColor" /> : <Play size={26} fill="currentColor" />}</span></button>
                  <button aria-label="Play next song" title="Next song" onClick={playNextSong} disabled={!activeSong || !filteredSongs.length} className="grid h-12 w-12 place-items-center rounded-full bg-white/10 text-white transition hover:scale-105 hover:bg-white/15 disabled:cursor-not-allowed disabled:opacity-40"><SkipForward size={18} /></button>
                  <button aria-label={likedSongIds.includes(activeSong?.id ?? "") ? "Unlike current song" : "Like current song"} title={likedSongIds.includes(activeSong?.id ?? "") ? "Unlike current song" : "Like current song"} onClick={toggleLike} disabled={!activeSong} className={`grid h-12 w-12 place-items-center rounded-full border transition ${activeSong && likedSongIds.includes(activeSong.id) ? "border-[#f5bd6d] bg-[#f5bd6d]/20 text-[#f5bd6d]" : "border-white/20 bg-white/5 text-white/70 hover:border-white/35 hover:bg-white/10"}`}><Heart size={18} fill={activeSong && likedSongIds.includes(activeSong.id) ? "currentColor" : "none"} /></button>
                </div>
              </div>
              <div className="flex items-center justify-between text-[11px] font-semibold uppercase tracking-[0.16em] text-white/70">
                <span>{formatTime(currentTime)}</span>
                <span>{activeSong ? "Live mix" : "Ready"}</span>
                <span>{formatTime(duration)}</span>
              </div>
            </div>
          </div>
